...
import actDashShowQuizLesson from '@store/dashboard/actquizLesson/actDashShowQuizLesson';
import actDashUpdateQuizLesson from '@store/dashboard/actquizLesson/actDashUpdateQuizLesson';
...
function Edit(props: { courseId: number, userId: number, setUserEdited: () => void }) {
    const { quiz_lesson } = useAppSelector(state => state.dashboard)
    ...
    useEffect(() => {
        dispatch(actDashShowQuizLesson(props.userId!))


    }, [dispatch, props.userId])
    useEffect(() => {
        if (quiz_lesson) {
            setQuestion(quiz_lesson?.question)
            setAnswer_1(quiz_lesson?.answer_1)
            setAnswer_2(quiz_lesson?.answer_2)
            setAnswer_3(quiz_lesson?.answer_3)
            setAnswer_4(quiz_lesson?.answer_4)
            setAnswerRight(quiz_lesson?.answer_right)
        }
    }, [quiz_lesson])
    const data: TQuiz = {
        id: props.userId,
    ...
    const addNewUser = () => {
        ...
        dispatch(actDashUpdateQuizLesson(data)).unwrap().then(() => {
