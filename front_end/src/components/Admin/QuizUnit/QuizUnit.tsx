...
import ViewUser from '@components/Admin/QuizUnitOperations/View/View';
import AddUser from '@components/Admin/QuizUnitOperations/Add/Add';
import EditUser from '@components/Admin/QuizUnitOperations/Edit/Edit';
...
import actDashGetQuizUnit from '@store/dashboard/actquizUnit/actDashGetQuizUnit';
import actDashDeleteQuizUnit from '@store/dashboard/actquizUnit/actDashDeleteQuizUnit';
type TQuizTest = {
    id: number,
    question: string,
    answer_1: string,
...
function QuizUnit({ Id }: { Id: number }) {
    const { language } = useAppSelector(state => state.language);
    const { quiz_units } = useAppSelector(state => state.dashboard);
    ...
    useEffect(() => {
        dispatch(actDashGetQuizUnit(Id!))

    }, [dispatch])
    useEffect(() => {
        if (quiz_units?.length) {
            const quiz: TQuizTest[] = quiz_units?.map((course) => {
                return ({
                    id: course.id!,
            ...
            const quizAra: TQuizTestAra[] = quiz_units?.map((course) => {
                return ({
                    id: course.id!,
    ...
    const deleteUser = (userId: number) => {
        dispatch(actDashDeleteQuizUnit(userId)).unwrap().then(() => {
