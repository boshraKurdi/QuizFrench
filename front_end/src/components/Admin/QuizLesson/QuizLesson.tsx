...
import ViewUser from '@components/Admin/QuizLessonOperations/View/View';
import AddUser from '@components/Admin/QuizLessonOperations/Add/Add';
import EditUser from '@components/Admin/QuizLessonOperations/Edit/Edit';
...
import actDashGetQuizLesson from '@store/dashboard/actquizLesson/actDashGetQuizLesson';
import actDashDeleteQuizLesson from '@store/dashboard/actquizLesson/actDashDeleteQuizLesson';
type TQuizTest = {
    id: number,
    question: string,
    answer_1: string,
...
function QuizLesson({ Id }: { Id: number }) {
    const { language } = useAppSelector(state => state.language);
    const { quiz_lessons } = useAppSelector(state => state.dashboard);
    ...
    useEffect(() => {
        dispatch(actDashGetQuizLesson(Id!))

    }, [dispatch])
    useEffect(() => {
        if (quiz_lessons?.length) {
            const quiz: TQuizTest[] = quiz_lessons?.map((course) => {
                return ({
                    id: course.id!,
            ...
            const quizAra: TQuizTestAra[] = quiz_lessons?.map((course) => {
                return ({
                    id: course.id!,
    ...
    const deleteUser = (userId: number) => {
        dispatch(actDashDeleteQuizLesson(userId)).unwrap().then(() => {
